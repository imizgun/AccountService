...
public class TransactionController(IMediator mediator) : ControllerBase
{
    ...
    /// <param name="cancellationToken"></param>
    /// <param name="requets">Тело c xmin</param>
    /// <returns>Сообщение, успешно ли прошло удаление</returns>
    /// <response code="200">Успешное удаление</response>
    /// <response code="400">Некорректный запрос или ошибка на сервере</response>
    /// <response code="401">Необходима авторизация</response>
    [HttpDelete("{transactionId:guid}")]
    public async Task<ActionResult<MbResult<Guid>>> DeleteTransaction(Guid transactionId, XminRequets requets, CancellationToken cancellationToken)
    {
        var command = new DeleteTransactionCommand(transactionId, requets.xmin);
